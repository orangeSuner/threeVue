...
<script setup>
  ...
  onMounted(()=> {
    ...
    // 创建一个立方体
    const cubeGeometry = new THREE.BoxGeometry(10,12,22)
    const cubeMaterial = new THREE.MeshLambertMaterial({color: 0xee8822, wireframe: true})
    ...
    let obj = null
    gltfLoader.load('/gltf/model.gltf', (model)=> {
      console.log(model)
      obj = model.scene.children[0]
      obj.position.set(50, 30, 30)
      scene.add(obj)
      console.log(scene.children)

      obj.children[0].material.color.set('#ff6800')
    })
